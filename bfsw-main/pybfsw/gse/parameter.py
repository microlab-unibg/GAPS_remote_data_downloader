...
class Parameter:
    def __init__(
        self, name, table, column, where=None, converter=None, low_range=None, high_range=None,low_alarm=None,high_alarm=None,units="raw", comment=""
    ):
        """
    ...
    def convert(self, y):
        if isinstance(self.converter, str):
            for c in self.converter:
                if c not in chars:
                    raise ValueError("bad eval string, valid chars are {chars}")
